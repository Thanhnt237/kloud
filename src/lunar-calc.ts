...
export enum CalenderConvertTypeEnum {
...
export function calenderConvert(date: Date, convertType: CalenderConvertTypeEnum){
...
function convertLunarCalendarToSolarCalendar(lunarDay: number, lunarMonth: number, lunarYear: number, lunarLeap: number){
...
function convertSolarCalendarToLunarCalendar(solarDay: number, solarMonth: number, solarYear: number){
...
function calendarToJuliusDay(day: number, month: number, year: number){
...
function juliusDayToCalendar(jd: number){
    let a, b, c;
...
export const calcNumerology = (dobDate: Date) => {
    if (!dobDate) return {
        totalScore: null,
    ...
    try {
        ...
        while (dobScore > 11){
            if(dobScore === 22) isSpecialNumerology = true;
